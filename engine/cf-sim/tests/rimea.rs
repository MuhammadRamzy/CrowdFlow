...
fn ring_walls(n: usize, doors: &[(usize, usize)]) -> Vec<(usize, usize)> {
    ring(n)
        .into_iter()
        .filter(|e| !doors.iter().any(|d| edge_key(d.0, d.1) == edge_key(e.0, e.1)))
        .collect()
}
...
fn tc7_a_population_reproduces_its_specified_speed_distribution() {
    ...
    assert!((mean - 1.34).abs() < 0.02, "mean walking speed {mean:.4} m/s");
...
fn tc12_two_streams_merge_without_starving_either_branch() {
    ...
    order.sort_by(|a, b| times[*a].partial_cmp(&times[*b]).expect("no NaN exit times"));
